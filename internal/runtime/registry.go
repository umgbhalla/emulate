...
var Services = []Service{
	...
	{Name: "discord", Label: "Discord REST API emulator", Endpoints: "bot auth, users, guilds, channels, members, messages, inspector", InitConfig: map[string]any{"guild": map[string]any{"name": "My Discord Server"}, "bot": map[string]any{"username": "my-bot", "token": "test-token"}, "users": []map[string]any{{"username": "developer", "global_name": "Developer", "email": "dev@example.com"}}, "channels": []map[string]any{{"name": "general", "topic": "General discussion"}, {"name": "ops", "topic": "Ops alerts"}}}},
	...
	{Name: "aws", Label: "AWS cloud service emulator", Endpoints: "S3 (buckets, objects), SQS (queues, messages), SNS (topics, subscriptions), EventBridge (buses, rules, targets), DynamoDB (tables, items), CloudWatch Logs (groups, streams, events), Secrets Manager (secrets, versions), SSM Parameter Store (parameters, versions), KMS (keys, aliases, local ciphertext stubs), IAM (users, roles, access keys), STS (assume role, caller identity)", InitConfig: map[string]any{"region": "us-east-1", "s3": map[string]any{"buckets": []map[string]any{{"name": "my-app-bucket"}, {"name": "my-app-uploads"}}}, "sqs": map[string]any{"queues": []map[string]any{{"name": "my-app-events"}, {"name": "my-app-dlq"}}}, "secretsmanager": map[string]any{"secrets": []map[string]any{{"name": "my-app/database-url", "secret_string": "postgres://localhost:5432/app"}}}, "ssm": map[string]any{"parameters": []map[string]any{{"name": "/my-app/database-url", "type": "SecureString", "value": "postgres://localhost:5432/app"}}}, "kms": map[string]any{"keys": []map[string]any{{"description": "My app KMS key", "aliases": []string{"alias/my-app"}}}}, "iam": map[string]any{"users": []map[string]any{{"user_name": "developer", "create_access_key": true}}, "roles": []map[string]any{{"role_name": "lambda-execution-role", "description": "Role for Lambda function execution"}}}}},
