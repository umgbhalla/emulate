...
- `GET /_emulate/discord/application` - returns local app wiring metadata, including the public key
- `POST /_emulate/discord/interactions` - sends a signed slash command interaction to `target_url`

```bash
curl -s http://localhost:4003/_emulate/discord/application
...
Use the returned `public_key` as your app's Discord public key. The simulator signs the JSON payload and creates the matching followup webhook for `/api/v10/webhooks/:applicationId/:interactionToken`.
...
Gateway WebSocket connections are not implemented in the native Discord engine yet.
